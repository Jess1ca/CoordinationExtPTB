...
from constituency_tree import sexprs
...
class LingTree:
   ...
   def get_subtree(self,id1,id2):
    l = list(self.search(lambda x: not x.is_leaf() and [y.id for y in x.collect_leaves(ignore_empties=False)] and id1 == min([y.id for y in x.collect_leaves(ignore_empties=False)]) and id2 == max([y.id for y in x.collect_leaves(ignore_empties=False)])))
    if not l :
        return list(self.search(lambda x:x.is_leaf() and x.id==id1))[0] if id1==id2 else None
    max_nodes = len(list(l[0].search(lambda x:x)))
    ret = l[0]
    for n in l:
        y = len(list(n.search(lambda x:x)))
        if y>max_nodes:
            max_nodes = y
            ret = n
    return ret
